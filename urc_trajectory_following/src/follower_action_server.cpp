#include "urc_trajectory_following/follower_action_server.hpp"
#include "geometry_msgs/msg/transform_stamped.hpp"
#include "urc_trajectory_following/geometry_util.hpp"
...
: Node("follower_action_server", options)
{
  ...
  declare_parameter("trajectory_controller", "pure_pursuit");

  const auto controller_type = get_parameter("trajectory_controller").as_string();
  if (!trajectory_following::TrajectoryFactory::supports(controller_type)) {
  ...
  costmap_subscriber_ = create_subscription<grid_map_msgs::msg::GridMap>(
    ...
    std::bind(
      &FollowerActionServer::handleCostmap, this,
      std::placeholders::_1));
  ...
  navigate_server_ =
    ...
    std::bind(
      &FollowerActionServer::handle_navigate_goal, this,
      std::placeholders::_1, std::placeholders::_2),
    std::bind(
      &FollowerActionServer::handle_navigate_cancel, this,
      std::placeholders::_1),
    std::bind(
      &FollowerActionServer::handle_navigate_accepted, this,
      std::placeholders::_1));
...
FollowerActionServer::lookup_transform(
  ...
  std::string source_frame)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      this->get_logger(), "Could not lookup transform: %s",
      ex.what());
  }
  return transform;
}
...
{
  std::thread{std::bind(
      &FollowerActionServer::execute_navigate, this,
      std::placeholders::_1),
    goal_handle}
  .detach();
}
  ...
  const geometry_msgs::msg::PoseStamped & goal, bool & success)
{
  ...
  if (result.wait_for(std::chrono::seconds(10)) == std::future_status::ready) {
    ...
    {
      RCLCPP_INFO(
        this->get_logger(), "Planning successful, path has %ld poses",
        response->path.poses.size());
      success = true;
      return response->path;
    } else {
      RCLCPP_ERROR(
        this->get_logger(), "Planning failed with error code %d",
        response->error_code);
      success = false;
  ...
  goal_handle)
{
  ...
  if (goal_msg->has_goal) {
    ...
    try {
      ...
      RCLCPP_ERROR(
        this->get_logger(), "Could not get current pose: %s",
        ex.what());
      result->error_code =
  ...
  auto trajectory_controller = trajectory_following::TrajectoryFactory::create(
    get_parameter("trajectory_controller").as_string(), *this);
  trajectory_controller->setPath(path);
  RCLCPP_INFO(
    this->get_logger(), "Following path with %ld poses",
    path.poses.size());

  trajectory_following::TrajectoryOutput output;
  rclcpp::Rate rate(10);
  ...
  while (rclcpp::ok()) {
      ...
      get_parameter("goal_tolerance").as_double())
    {
        ...
        if (!heading_satisfied) {
          ...
          if (get_parameter("enable_swerve_motion").as_bool()) {
            ...
            align_cmd.twist.angular.z =
              std::clamp(
              signed_error * 2.0,              // Proportional control
      ...
      if (heading_satisfied) {
        ...
        output.tracking_point.point.x,
        output.tracking_point.point.y) >
    ...
    output = trajectory_controller->getCommandVelocity(
      this->get_logger(),
      lookup_transform(
        get_parameter("base_link_frame").as_string(),
        get_parameter("map_frame").as_string()));
