...
trajectory_following::TrajectoryOutput PurePursuit::getCommandVelocity(
  const rclcpp::Logger & logger,
  const geometry_msgs::msg::TransformStamped & map_to_base_link)
{
  ...
  if (params_.enable_swerve_motion) {
    ...
    double lookahead_angle = std::atan2(
      ...
      std::abs(lookahead_angle) > params_.heading_alignment_tolerance)
    {
      ...
      cmd_vel.twist.angular.z =
        std::clamp(
        linear_vel * curvature, -params_.max_angular_velocity,
  ...
  output.tracking_point = lookahead_point;
