  ...
  tfListener_(tfBuffer_)
{
  ...
  // publishes the ground truth state
...
void GroundTruth::groundTruthCallback(const nav_msgs::msg::Odometry & msg)
{
  ...
  if (g_og_pose.header.stamp.sec == 0) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "setting g_og_pose to %f,%f,%f", g_og_pose.pose.pose.position.x, g_og_pose.pose.pose.position.y,
    ...
    result.pose.pose.position.z = msg.pose.pose.position.z - g_og_pose.pose.pose.position.z +
    ...
    if (std::fabs(msg.header.stamp.sec - g_last_estimate.seconds()) > 1.0) {
      ...
      transform_stamped =
        this->tfBuffer_.lookupTransform("odom", "base_footprint", rclcpp::Time(0));
