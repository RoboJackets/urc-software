...
PLUGINLIB_EXPORT_CLASS(
  urc_hardware::hardware_interfaces::TestHardware,
...
hardware_interface::CallbackReturn TestHardware::on_init(
  const hardware_interface::HardwareInfo & info)
{
  RCLCPP_INFO(
    rclcpp::get_logger(
      "TestHardware"), "initializing hardware for robot %s..", info_.name.c_str());
  ...
  if (info_.hardware_parameters.find("serial_port") == info_.hardware_parameters.end()) {
    RCLCPP_ERROR(
      rclcpp::get_logger("TestHardware"),
  ...
  // check joints
  test_joint_position_commands_.resize(
    info.joints.size(),
  ...
  if (info_.gpios[0].command_interfaces[0].name != urc_hardware::types::HW_CMD) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "TestHardware"), "Error during initlization: status indication should be HW_CMD.");
    return hardware_interface::CallbackReturn::ERROR;
  }
...
hardware_interface::CallbackReturn TestHardware::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO_ONCE(
    rclcpp::get_logger(
      "TestHardware"), "Succesfully zeroed all command on configuration.");
  return hardware_interface::CallbackReturn::SUCCESS;
}
...
std::vector<hardware_interface::StateInterface> TestHardware::export_state_interfaces()
{
  ...
  for (unsigned long i = 0; i < info_.joints.size(); ++i) {
    ...
    state_interfaces.emplace_back(
      component.name, urc_hardware::types::HW_POSITION,
      &this->test_joint_position_states_[i]);
    state_interfaces.emplace_back(
      component.name, urc_hardware::types::HW_VELOCITY,
      &this->test_joint_velocity_states_[i]);
  }
  state_interfaces.emplace_back(
    info_.sensors[0].name, "quaternion.w",
    &this->imu_quaternion_readings_[0]);
  state_interfaces.emplace_back(
    info_.sensors[0].name, "quaternion.x",
    &this->imu_quaternion_readings_[1]);
  state_interfaces.emplace_back(
    info_.sensors[0].name, "quaternion.y",
    &this->imu_quaternion_readings_[2]);
  state_interfaces.emplace_back(
    info_.sensors[0].name, "quaternion.z",
    &this->imu_quaternion_readings_[3]);
...
std::vector<hardware_interface::CommandInterface> TestHardware::export_command_interfaces()
{
  ...
  command_interfaces.emplace_back(
    info_.gpios[0].name, urc_hardware::types::HW_CMD,
    &this->status_light_cmd);
