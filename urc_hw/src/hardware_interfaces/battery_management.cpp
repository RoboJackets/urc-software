  ...
  for (auto interface : ptr->state_interfaces)
  {
    ...
    else if (INTERFACE_NAMES.find(interface.name) != INTERFACE_NAMES.end())
    {
      state_interfaces.emplace_back(
          ...
          &this->core_status[std::distance(INTERFACE_NAMES.begin(), INTERFACE_NAMES.find(interface.name))]);
    }
  }
...
void BatteryManagement::decode(const char* buf, ssize_t size)
{
  ...
  if (!status)
  {
    RCLCPP_INFO(rclcpp::get_logger(hardware_interface_name), "Decoding failed: %s\n", PB_GET_ERROR(&stream));
  }
