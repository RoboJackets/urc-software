...
public:
  // Event Listeners:
  std::function<void(TCPSocket<BUFFER_SIZE> *)> onNewConnection =
    [](TCPSocket<BUFFER_SIZE> * sock) {
      FDR_UNUSED(sock)
    };

  explicit TCPServer(FDR_ON_ERROR)
  : BaseSocket(onError, SocketType::TCP)
  {
    int opt = 1;
  ...
  static void Accept(TCPServer<BUFFER_SIZE> * server, FDR_ON_ERROR)
  {
    ...
    while (true) {
      newSocketFileDescriptor = accept(
        server->sock, (sockaddr *)&newSocketInfo,
        &newSocketInfoLength);
      ...
      TCPSocket<BUFFER_SIZE> * newSocket = new TCPSocket<BUFFER_SIZE>(
        onError,
