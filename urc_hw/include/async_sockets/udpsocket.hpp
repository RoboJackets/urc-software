...
public:
  ...
  ssize_t SendTo(
    const char * bytes, size_t byteslength, const char * host, uint16_t port,
    FDR_ON_ERROR)
  {
    ...
    ssize_t sent_length =
      sendto(this->sock, bytes, byteslength, 0, (sockaddr *)&hostAddr, sizeof(hostAddr));
    if (sent_length == -1) {
  ...
  // Send raw bytes to a spesific `host` & `port` with no connection
  ssize_t SendTo(
    const char * bytes, size_t byteslength, const std::string & host, uint16_t port,
    FDR_ON_ERROR)
  {
    ...
    while ((messageLength = recv(udpSocket->sock, tempBuffer, BUFFER_SIZE, 0)) != -1) {
      ...
      if (udpSocket->onMessageReceived) {
        udpSocket->onMessageReceived(
          std::string(tempBuffer, messageLength),
      ...
      if (udpSocket->onRawMessageReceived) {
        udpSocket->onRawMessageReceived(
          tempBuffer, messageLength, ipToString(
            udpSocket->address), ntohs(udpSocket->address.sin_port));
      }
    }
  }
  ...
  static void ReceiveFrom(UDPSocket * udpSocket)
  {
    ...
    while ((messageLength =
      recvfrom(
        udpSocket->sock, tempBuffer, BUFFER_SIZE, 0, (sockaddr *)&hostAddr,
        &hostAddrSize)) != -1)
    {
      tempBuffer[messageLength] = '\0';
      if (udpSocket->onMessageReceived) {
        udpSocket->onMessageReceived(
          std::string(tempBuffer, messageLength), ipToString(
            hostAddr), ntohs(hostAddr.sin_port));
      }

      if (udpSocket->onRawMessageReceived) {
        udpSocket->onRawMessageReceived(
          tempBuffer, messageLength, ipToString(hostAddr),
