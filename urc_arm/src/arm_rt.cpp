...
void ArmRT::initialize()
{
  ...
  joint_state_subscription_ = create_subscription<sensor_msgs::msg::JointState>(
      ...
      [this](const sensor_msgs::msg::JointState& joint_state) { joint_state_buffer_.writeFromNonRT(joint_state); });

  vel_command_publisher_ = std::make_shared<realtime_tools::RealtimePublisher<std_msgs::msg::Float64MultiArray>>(
      create_publisher<std_msgs::msg::Float64MultiArray>(vel_command_topic_, rclcpp::SystemDefaultsQoS()));
...
void ArmRT::load_model(const std::string& decsription)
{
  ...
  for (std::vector<std::string>::const_iterator it = locking_joints_.begin(); it != locking_joints_.end(); ++it)
  {
    const std::string& joint_name = *it;
    if (full_model.existJointName(joint_name))
      joint_to_lock_by_id.push_back(full_model.getJointId(joint_name));
    else
      RCLCPP_ERROR(*logger_, "Joint %s does not belong to the model. Skipping locking.", joint_name.c_str());
  }

  model_ = std::make_shared<Model>(pinocchio::buildReducedModel(full_model, joint_to_lock_by_id, q_neutral));
  Data data(*model_);
...
{
  return std::distance(joint_list.begin(), std::find(joint_list.begin(), joint_list.end(), joint_name));
}
...
void ArmRT::update_command()
{
  ...
  delta.rotation() = rpy_to_rot_mat(vel_target->angular.x, vel_target->angular.y, vel_target->angular.z);
...
double ArmRT::calc_joint_limit_scaling_factor(WalliArmState& joint_commands_)
{
  ...
  {
    double bounded_vel = std::clamp(joint_commands_(i, 0), joint_velocity_limits_[i], -joint_velocity_limits_[i]);
