...
namespace urc_slam {
    ...
    void SlamBackend::initialize(const gtsam::Pose3 &initial_pose) {
        ...
        trajectory.clear();
        trajectory.push_back(last_estimated_pose);
    }
    ...
    bool SlamBackend::addKeyframe(const gtsam::Pose3 &measured_pose) {
    ...
    std::vector<gtsam::Pose3> SlamBackend::trajectory() const {
        return trajectory;
    }
