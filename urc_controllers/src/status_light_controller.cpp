...
PLUGINLIB_EXPORT_CLASS(urc_controllers::StatusLightController, controller_interface::ControllerInterface);
...
controller_interface::CallbackReturn StatusLightController::on_configure(const rclcpp_lifecycle::State&)
{
...
controller_interface::CallbackReturn StatusLightController::on_activate(const rclcpp_lifecycle::State&)
{
  for (auto& interface : command_interfaces_)
  {
    const auto interface_ptr =
        std::find(STATUS_LIGHT_INTERFACES.begin(), STATUS_LIGHT_INTERFACES.end(), interface.get_interface_name());
    if (interface_ptr == STATUS_LIGHT_INTERFACES.end())
      continue;

    command_interface_map.emplace(
        interface.get_interface_name(),
        std::make_shared<std::reference_wrapper<hardware_interface::LoanedCommandInterface>>(interface));
  }
  ...
  color_command_subscriber_ = get_node()->create_subscription<std_msgs::msg::Int8>(
      "/cmd_statuslight_color", rclcpp::SystemDefaultsQoS(), [this](std::shared_ptr<std_msgs::msg::Int8> message) {
  ...
  display_command_subscriber_ = get_node()->create_subscription<std_msgs::msg::Int8>(
      "/cmd_statuslight_display", rclcpp::SystemDefaultsQoS(), [this](std::shared_ptr<std_msgs::msg::Int8> message) {
...
controller_interface::CallbackReturn StatusLightController::on_deactivate(const rclcpp_lifecycle::State&)
{
...
controller_interface::CallbackReturn StatusLightController::on_cleanup(const rclcpp_lifecycle::State&)
{
  color_command_subscriber_.reset();
  display_command_subscriber_.reset();
  return controller_interface::CallbackReturn::SUCCESS;
...
controller_interface::CallbackReturn StatusLightController::on_error(const rclcpp_lifecycle::State&)
{
  color_command_subscriber_.reset();
  display_command_subscriber_.reset();
  return controller_interface::CallbackReturn::SUCCESS;
...
controller_interface::CallbackReturn StatusLightController::on_shutdown(const rclcpp_lifecycle::State&)
{
  color_command_subscriber_.reset();
  display_command_subscriber_.reset();
  return controller_interface::CallbackReturn::SUCCESS;
...
controller_interface::return_type StatusLightController::update(const rclcpp::Time&, const rclcpp::Duration&)
{
