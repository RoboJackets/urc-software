  ...
  double * max_x, double * max_y)
{
  ...
  if (rolling_window_) {
    costmap_2d_.updateOrigin(
      robot_x - costmap_2d_.getSizeInMetersX() / 2,
...
void TraversabilityLayer::matchCostmapDims(const nav2_costmap_2d::Costmap2D & master_grid)
{
  ...
  bool different_dims = costmap_2d_.getSizeInCellsX() != cells_x ||
    costmap_2d_.getSizeInCellsY() != cells_y ||
...
void TraversabilityLayer::updateRollingWindow()
{
  ...
  grid_map::Position costmap_tl_corner =
    costmap_br_corner +
    grid_map::Position{costmap_2d_.getSizeInMetersX(), costmap_2d_.getSizeInMetersY()};
