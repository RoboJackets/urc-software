...
// , move_group_node_{ std::make_shared<rclcpp::Node>("move_group_node", options) }
{
  ...
  pose_tracking_ = std::make_unique<moveit_servo::PoseTracking>(
    node_, servo_parameters,
...
void ArmRTServer::moveToPoseUpdate()
{
  for (;; ) {
    ...
    if (!pose_tracking_pause) {
      ...
      pose_tracking_->moveToPose(
        Eigen::Vector3d(config_parameters[0], config_parameters[0], config_parameters[0]),
