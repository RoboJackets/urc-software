...
namespace costmap_generator
{
    ...
    void CostmapGenerator::DepthCallback(const sensor_msgs::msg::Image & msg)
    {
        ...
        if (robotGridY != costmap.metadata.size_y - 1) { // Direction 0
            costmap.data[(robotGridY + 1) * costmap.metadata.size_x + (robotGridX + 0)] = std::max(0, data[0]);
        }
        if (robotGridY != costmap.metadata.size_y - 1 && robotGridX != costmap.metadata.size_x - 1) {  // Direction 1
            costmap.data[(robotGridY + 1) * costmap.metadata.size_x + (robotGridX + 1)] = std::max(0, data[1]);
        }
        if (robotGridX != costmap.metadata.size_x - 1) {  // Direction 2
            costmap.data[(robotGridY + 0) * costmap.metadata.size_x + (robotGridX + 1)] = std::max(0, data[2]);
        }
        if (robotGridY != 0 && robotGridX != costmap.metadata.size_x - 1) {  // Direction 3
            costmap.data[(robotGridY - 1) * costmap.metadata.size_x + (robotGridX + 1)] = std::max(0, data[3]);
        }
        if (robotGridY != 0) { // Direction 4
            costmap.data[(robotGridY - 1) * costmap.metadata.size_x + (robotGridX + 0)] = std::max(0, data[4]);   
        }
        if (robotGridY != 0 && robotGridX != 0) { // Direction 5
            costmap.data[(robotGridY - 1) * costmap.metadata.size_x + (robotGridX - 1)] = std::max(0, data[5]);   
        }
        if (robotGridX != 0) { // Direction 6
            costmap.data[(robotGridY + 0) * costmap.metadata.size_x + (robotGridX - 1)] = std::max(0, data[6]);   
        }
        if (robotGridY != costmap.metadata.size_y - 1 && robotGridX != 0) { // Direction 7
            costmap.data[(robotGridY + 1) * costmap.metadata.size_x + (robotGridX - 1)] = std::max(0, data[7]);   
        }
