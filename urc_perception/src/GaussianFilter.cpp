...
bool GaussianFilter<T>::configure()
{
  ...
  if (gaussian_coeff_ < 0.0) {
    RCLCPP_ERROR(
      this->logging_interface_->get_logger(), "Gaussian coefficient must be greater than zero.");
    return false;
  }

  RCLCPP_DEBUG(
    this->logging_interface_->get_logger(), "Gaussian coefficient = %f.",
    gaussian_coeff_);
...
PLUGINLIB_EXPORT_CLASS(
  urc_perception::GaussianFilter<grid_map::GridMap>,
